...
const layout2 = new ObjectAbstract({
    entities : new ArrayAbstract(null,
        new ObjectAbstract({
    ...
    options : new ObjectAbstract({}, true)
})
...
export default class Deserializer {
    ...
    async generate () {
        ...
        let entities = [];
        for (let key in this.json.entities) {
            let source = this.json.entities[key];

            entities.push(await new Entity(source.transform, source.geometry, source.renderer, source.scripts));
        }

        return { entities, options : this.json.options };
    }
}
